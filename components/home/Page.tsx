...
export default function Page() {
  ...
  const handleDelete = (index: number) => {
    if(confirm('Are you sure you want to delete this'))setPosts(posts => posts.filter((post, i) => i !== index ))
  }
  console.log(1);
            ...
            {posts.map(({ comment, name, dateAdded }, index) => {
              return (
                <Tr key={index}>
                  ...
                  <Td>{dateAdded}</Td>
                  <Td>
                    <Button color='red300' variant="solid" onClick={() => handleDelete(index)}>
